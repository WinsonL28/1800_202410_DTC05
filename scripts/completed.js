...
function getWorkoutHistory(user) {
    db.collection("users").doc(user.uid).get().then(userDoc => {
        var completedwo = userDoc.data().WorkoutHistory;
        console.log(completedwo)
        let newcardTemplate = document.getElementById("CompletedCardTemplate")
        
        completedwo.forEach(workoutID => {
